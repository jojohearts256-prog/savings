...
export default function MemberDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      {/* Navbar */}
      <nav className="bg-gradient-to-r from-[#007B8A] via-[#00BFFF] to-[#D8468C] p-4 flex justify-between items-center text-white rounded-xl mb-6">
        ...
        {showNotifications && (
          <div className="absolute right-4 top-14 w-80 max-h-96 overflow-y-auto bg-gray-800 text-white rounded-xl shadow-lg p-4 z-50">
            {notifications.length === 0 ? <p className="text-sm text-gray-300">No notifications</p> :
              notifications.map(n => <div key={n.id} className={`p-2 rounded-xl mb-2 ${n.read ? 'bg-gray-700' : 'bg-orange-500'}`}><p className="text-sm">{n.message}</p><p className="text-xs text-gray-200">{new Date(n.sent_at).toLocaleString()}</p></div>)
            }
          </div>
        )}
      </nav>

      {/* Dashboard Cards (Transactions + Loans) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-4 rounded-2xl shadow flex flex-col">
          <DollarSign className="text-green-500 mb-2" />
          <p className="text-sm text-gray-500">Account Balance</p>
          <p className="text-xl font-bold">{member?.account_balance.toLocaleString()} UGX</p>
        </div>

        <div className="bg-white p-4 rounded-2xl shadow flex flex-col">
          <TrendingUp className="text-orange-500 mb-2" />
          <p className="text-sm text-gray-500">Total Contributions</p>
          <p className="text-xl font-bold">{member?.total_contributions.toLocaleString()} UGX</p>
        </div>

        <div className="bg-white p-4 rounded-2xl shadow flex flex-col">
          <CreditCard className="text-blue-500 mb-2" />
          <p className="text-sm text-gray-500">Pending Loans</p>
          <p className="text-xl font-bold">{loans.filter(l => l.status==='pending').length}</p>
        </div>

        {/* Transactions Cards */}
        {transactions.map(t => (
          <div key={t.id} className="bg-white p-4 rounded-2xl shadow flex flex-col hover:shadow-lg transition">
        ...
        {/* Loans Cards */}
        {loans.map(l => (
          <div key={l.id} className="bg-white p-4 rounded-2xl shadow flex flex-col hover:shadow-lg transition">
