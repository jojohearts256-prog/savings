...
export default function ProfitDistribution() {
  ...
  const handleDisburse = async (memberId: number) => {
    ...
    try {
      ...
      // Update member status in UI
      setMembers(prev => prev.map(m => m.id === memberId ? { ...m, account_balance: newBalance, status: 'Paid' } : m));
  ...
  const chartData = members.map(m => ({
    ...
    percent: m.sharePercent ? +(m.sharePercent * 100).toFixed(2) : 0,
  }));
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData}>
            ...
            <Legend />
            <Bar dataKey="profit" fill="#10B981" name="Profit UGX" />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
                ...
                <tr key={m.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-center">
                    <button
                      ...
                      disabled={distributing || !m.profitShare || m.profitShare <= 0 || m.status === 'Paid'}
                      className="px-4 py-1 btn-primary text-white rounded-xl disabled:opacity-50"
                    >
                      {distributing && m.status !== 'Paid' ? 'Processing...' : 'Disburse'}
                    </button>
                  </td>
                </tr>
              ))}
              ...
              <tr className="bg-gray-100 font-semibold text-gray-800">
                ...
                <td className="px-6 py-4">+UGX {totalBalances.toLocaleString()}</td>
                <td className="px-6 py-4"></td>
                <td className="px-6 py-4">+UGX {totalProfit.toLocaleString()}</td>
                <td className="px-6 py-4"></td>
                <td className="px-6 py-4"></td>
              </tr>
            </tbody>
